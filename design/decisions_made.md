...
rethemed to dark mode and given a user-facing `DASHBOARD_CONFIG` JavaScript object at
the top of its `<script>` block. All structural colors, font settings, and staleness
thresholds are exposed through this block. CSS custom properties (`:root { --var }`)
carry the values into the stylesheet; `applyConfig()` runs immediately on script load to
...
inline constants) because they are: (1) native to the browser with no build step, (2)
overridable at runtime by JS, (3) consistent with the dashboard's single-file
constraint. The CONFIG-first approach means wiki-dashboard.py regenerates the HTML data
...
- CSS-only theming (no JS CONFIG): thresholds (urgentDays, watchDays, dashboardStaleDays)
  cannot be expressed in CSS; would require a split approach anyway.

**Consequences to Watch:**
- wiki-dashboard.py regenerates the WIKI_DATA blob in-place; it must not overwrite the
  DASHBOARD_CONFIG block. Confirm that the Python script's HTML template preserves
  content above the `const WIKI_DATA` line on each regeneration.
- The dark-mode badge colors (typeBadge, statusBadge, credBadge maps) are hardcoded in
  the JS helpers, not in DASHBOARD_CONFIG. If badge colors need to be user-configurable
  in a future iteration, they should be promoted into DASHBOARD_CONFIG.colors.
- Three-script sync rule (DM-107) still applies: vocabulary changes must update
  wiki-dashboard.py, wiki-lint.py, and wiki-verify.sh. The HTML template is a separate
  artifact and is not in scope for that rule.
