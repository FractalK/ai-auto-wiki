...
| CC-02 | Claude Code can fetch URLs over the network during ingest | OPERATIONS.md Section 11.2 Step 1 — URL intake from queue.md | unverified | Test a URL fetch from a known institutional domain (e.g., anthropic.com/news) before first ingest session; confirm no proxy or network policy blocks egress |
| CC-03 | URL fetch failures are surfaced as errors, not silent nulls | OPERATIONS.md Section 11.2 — "[fetch-failed]" tagging logic | requires implementation test | Attempt fetch of a known bad URL; verify Claude Code reports the failure explicitly rather than returning empty content |
...
| CC-05 | Claude Code can read the full index.md in a single pass during query and lint operations | OPERATIONS.md Section 11.4 Step L2, OPERATIONS.md Section 11.5 Step Q2 | requires implementation test | At ~150 pages, index.md will be substantial. Test index.md read performance before the wiki reaches 100 pages. |
...
| CC-11 | YouTube transcript extraction tooling is available | OPERATIONS.md Section 11.2 Step 1 — "transcript extraction tool to be confirmed at implementation time" | unverified | Before ingesting any youtube-video source: confirm a transcript tool (e.g., yt-dlp with --write-auto-subs, or a manual export workflow). Document the chosen tool in OPERATIONS.md Section 11.2 before first YouTube ingest. Do not leave this as a runtime decision. |
...
| CC-14 | OPERATIONS.md exists in the wiki root before every wiki maintenance session | CLAUDE.md Section 1 — hard gate instruction; OPERATIONS.md contains all operational workflows | unverified | Copy OPERATIONS.md from design project output to wiki root before first ingest. CLAUDE.md Section 1 contains a hard gate: if OPERATIONS.md is not loaded, Claude Code outputs MISSING-OPERATIONS-FILE and halts. Verify OPERATIONS.md is present before every session start. |
...
| GIT-02 | queue.md syncs across machines via git | Section 8.4, OPERATIONS.md Section 11.2 — queue.md as cross-machine intake | confirmed (DM-019) | Confirm git remote is set up and the team has push/pull access before relying on queue.md for cross-machine coordination. |
...
| GIT-05 | raw/ directory is committed to git (not gitignored) | Section 2, OPERATIONS.md Section 11.3 — queue.md and discovery-sources.md are in raw/ | unverified | Confirm raw/ is tracked in git. If raw/staged/ contains large or copyright-restricted source files, add raw/staged/ to .gitignore while keeping raw/*.md tracked. |
...
12. Confirm YouTube transcript tooling and document in OPERATIONS.md Section 11.2 before first YouTube ingest (CC-11)
13. Obtain INIT-PROMPT.md and OPERATIONS.md from design project output before the Phase 1 initialization session (CC-13, CC-14). For all subsequent maintenance sessions, establish the session-start protocol using the prompt template in implementation-handoff.md Section 5. Confirm Claude Code reads both CLAUDE.md and OPERATIONS.md in full before taking any action on the first maintenance session. (CC-10, CC-13, CC-14)
