...
- **Symptom:** Full lint Phase 1 auto-compacted mid-batch during topic page reads at 100
  pages, before completing topic pages or reading tool, comparison, pitfalls, source, or
  teaching-brief pages. Steps L5a through L15 were not reached for any non-topic pages.
  The two schema violations found before compaction (ai-alignment and
  ai-workforce-complementarity, both at 6 Key Claims) are of uncertain fidelity in the
  compacted context. Proceeding to L13 from this state would produce a decision form with
  silently missing forced choices across all unread page types.
- **Verdict:** Confirmed gap — the context note was calibrated for ~60 pages and assumes
  the operator manually compacts at a clean step boundary before auto-compaction fires.
  At 100 pages, auto-compaction fires mid-batch before Phase 1 completes regardless of
  operator action. The existing guidance ("manual /compact is valid") does not address
  auto-compaction mid-batch, does not specify which steps to compact between, and provides
